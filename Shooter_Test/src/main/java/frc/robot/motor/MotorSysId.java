 ...
 * <p>Usage:
 * <pre>
 * MotorSysId sysId = new MotorSysId(motor, "Shooter/Preshooter", subsystem,
...
public class MotorSysId {
  ...
  private void stopCollectionAndAnalyze() {
    ...
    // Read all analysis parameters from dashboard + controller preset
    String prefix = m_motorName + "/SysId/";
    SysIdParams sysIdParams = SysIdParams.fromDashboard(prefix, m_preset);
  ...
  /** Publishes analysis results to SysId dashboard fields. */
  private void publishResults(SysIdAnalyzer.AnalysisResult result) {
    String prefix = m_motorName + "/SysId/";

    // Feedforward gains
    SmartDashboard.putNumber(prefix + "kS", round6(result.kS));
    SmartDashboard.putNumber(prefix + "kV", round6(result.kV));
    SmartDashboard.putNumber(prefix + "kA", round6(result.kA));
    SmartDashboard.putNumber(prefix + "kG", round6(result.kG));

    // Quality metrics
    SmartDashboard.putNumber(prefix + "R2_Accel", round6(result.rSquaredAccel));
    SmartDashboard.putNumber(prefix + "R2_SimVel", round6(result.rSquaredSimVel));
    SmartDashboard.putNumber(prefix + "RMSE", round6(result.rmse));
    SmartDashboard.putNumber(prefix + "Samples", result.sampleCount);

    // Feedback gains (from LQR)
    SmartDashboard.putNumber(prefix + "kP_vel", round6(result.kP_velocity));
    SmartDashboard.putNumber(prefix + "kP_pos", round6(result.kP_position));
    SmartDashboard.putNumber(prefix + "kD_pos", round6(result.kD_position));

    // Diagnostics
    SmartDashboard.putNumber(prefix + "AutoVelThresh", round6(result.autoVelocityThreshold));
    SmartDashboard.putNumber(prefix + "AutoQv", round6(result.autoQv));

    // LQR parameters used (write back so user can see what was applied)
    SmartDashboard.putNumber(prefix + "MaxEffort", result.maxControlEffort);
    SmartDashboard.putNumber(prefix + "MaxVelErr", result.maxVelocityError);
    SmartDashboard.putNumber(prefix + "MaxPosErr", result.maxPositionError);

    // Physical estimates
    SysIdAnalyzer.PhysicalEstimate est = result.physicalEstimate;
    if (est != null && est.valid) {
      SmartDashboard.putNumber(prefix + "Est_Inertia", round6(est.momentOfInertia));
      SmartDashboard.putNumber(prefix + "Est_Friction", round6(est.frictionTorque));
      SmartDashboard.putNumber(prefix + "Est_Mass_kg", round6(est.estimatedMass));
      SmartDashboard.putNumber(prefix + "Est_Efficiency", round6(est.efficiency));
      SmartDashboard.putNumber(prefix + "Est_MaxAccel", round6(est.maxAcceleration));
      SmartDashboard.putNumber(prefix + "Est_FreeSpeed", round6(est.freeSpeed));
    }
  ...
  /** Publishes status string to SmartDashboard. */
  private void publishStatus(String status) {
    SmartDashboard.putString(m_motorName + "/SysId/Status", status);
  }
