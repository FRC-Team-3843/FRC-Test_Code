...
import frc.lib.motor.config.MotorKind;
...
public class CanMotorWrapper implements UniversalMotor {
  ...
  public CanMotorWrapper(MotorConfiguration config) {
    ...
    switch (controllerType) {
      ...
      case TALON_SRX:
        ...
        break;
      case VICTOR_SPX:
        victorSpx = new WPI_VictorSPX(config.canId);
        configureVictorSpx(config);
        break;
      default:
  ...
  public double getAppliedVoltage() {
    switch (controllerType) {
      ...
      case VICTOR_SPX:
        return victorSpx.getMotorOutputPercent() * 12.0;
      default:
        return 0.0;
    }
  }
  ...
  private void configureVictorSpx(MotorConfiguration config) {
    victorSpx.setInverted(config.inverted);
    victorSpx.setNeutralMode(config.brakeMode ? NeutralMode.Brake : NeutralMode.Coast);
  }

  @Override
  public boolean isFeedbackConnected() {
    if (config.feedbackSource == FeedbackSource.NONE) {
      return false;
    }
    if (feedbackSensor != null) {
      return feedbackSensor.isConnected();
    }
    return config.feedbackSource == FeedbackSource.INTEGRATED && hasNativeIntegratedFeedback();
  }
  ...
  private boolean hasNativeIntegratedFeedback() {
